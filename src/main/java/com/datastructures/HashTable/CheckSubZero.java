...
// can you extract a contiguous sub-array from it which adds up to zero?

/*
The naive solution would be to iterate the array in a nested loop,
summing each element with all the elements succeeding it.
A hash table makes things much simpler.
We basically have to check for 3 conditions:

1. If 0 exists in the array
2. If the sum becomes zero in the iteration
3. If the sum reverts back to a value which was already a key in the hash table
