...
public class SubSetSum{
    ...
    The total complexity is : T(n) = 3T(n-1) + 1.
    If the tree is drawn, the process can be solved easily.
    Since it is called 3 times, the length of the tree has to go down to 3 above k.
    That's why complexity is O(3^n).
    The complexity of the algorithm written for Dynamic is shown below.
    ...
    /*
    The total complexity is : T(n) = 3(size x sum) +1 .
    This complexity also seems to be easy.
    The complexity is O(size x sum).
     Memoized's complexity is similar to that of dynamic. The difference
    between them is the use of maps and checking if cases with them.
...
*/
        ...
        initialTime= System.currentTimeMillis();
        int result = Memoized(set, set.length-1, sum , lookup);
        lastTime = System.currentTimeMillis();
        time = (double)(lastTime - initialTime)/1000;
